...
class RegisterViewModel @Inject constructor(private val repository: RegisterRepository) : ViewModel() {
    ...
    fun updateField(field: RegisterFormField, value: Any){
        when(field) {
            ...
            RegisterFormField.CAFFEINE_CONSUMPTION_FREQUENCY -> viewState = viewState.copy(
                registerForm = viewState.registerForm.copy(
                    caffeineConsumption = viewState.registerForm.caffeineConsumption.copy(frequency = Frequency.fromString(value as String))
                )
            )
            ...
            RegisterFormField.SMOKING_FREQUENCY -> viewState = viewState.copy(
                registerForm = viewState.registerForm.copy(
                    smoking = viewState.registerForm.smoking.copy(frequency = Frequency.fromString(value as String))
                )
            )
            RegisterFormField.ORAL_HABITS -> {
                @Suppress("UNCHECKED_CAST") val fieldValue = value as Pair<OralHabit, Boolean>
    ...
    fun submitForm() {
        repository.submitForm(viewState.registerForm)
    }
}
